...
const Petal = ({ position, rotation, scale, color, index, isBloom }: PetalProps) => {
  ...
  useFrame((state) => {
    ...
    // Bloom effect
    if (isBloom) {
      const bloomProgress = Math.min((state.clock.elapsedTime % 3) / 1.5, 1);
      meshRef.current.rotation.x = baseRotation.current[0] - bloomProgress * 0.3;
    }
  });
...
const Rose3D = ({ className = '' }: Rose3DProps) => {
  ...
  return (
    ...
    >
      ...
      {/* Click hint */}
      <div className="absolute bottom-4 left-1/2 -translate-x-1/2 text-rose-deep/60 text-sm font-sans animate-pulse">
        Click to bloom
      </div>
    </div>
  );
